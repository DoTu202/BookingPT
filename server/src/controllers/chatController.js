...
const chatController = {
  ...
  // Get user's chat rooms - simple version
  ...
  // Get messages - simple version
  getMessages: async (req, res) => {
    try {
      const { chatRoomId } = req.params;
      const userId = req.user._id;
      
      // Simple check: user must be PT or Client in this room
      const chatRoom = await ChatRoom.findById(chatRoomId);
      if (!chatRoom || (chatRoom.ptUser.toString() !== userId.toString() && chatRoom.clientUser.toString() !== userId.toString())) {
        return res.status(403).json({ success: false, message: 'Access denied' });
      }
      
      // Get messages - simple, no pagination for now
